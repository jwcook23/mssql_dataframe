'''Functions for preparing write operations and handling exceptions.'''
...
from mssql_dataframe.core import errors, dynamic, create, modify, conversion


def write_attempt(table_name, dataframe, statement, connection,
adjust_sql_attempts: int = 10, fast_executemany: bool = True):
    ...
    for idx in range(0, adjust_sql_attempts,1):
        error_class = errors.SQLGeneral("Generic SQL error in write.__attempt_write")
        try:
            conversion.insert_values(table_name, dataframe, connection, fast_executemany)
            # cursor.commit()
            break
        except (pyodbc.ProgrammingError, pyodbc.DataError, pyodbc.IntegrityError) as odbc_error:
            # cursor.rollback()
            error_class, undefined_columns = classify_error(table_name, dataframe, odbc_error)
            dataframe, error_class = handle_error(table_name, dataframe, error_class, undefined_columns)
    ...
    if idx==adjust_sql_attempts-1:
        raise RecursionError(f'adjust_sql_attempts={adjust_sql_attempts} reached')


def classify_error(table_name: str, dataframe: pd.DataFrame, odbc_error: pyodbc.Error):
    ...
    elif 'String data, right truncation' in error_string in error_string:
...
def handle_error(table_name: str, dataframe: pd.DataFrame, error_class: errors, undefined_columns: list):
...
def temp_table(table_name, match_columns, dataframe, operation: Literal['update','merge']):
