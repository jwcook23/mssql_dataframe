...
# from mssql_dataframe.core import errors, create, modify
...
#     def attempt(self, cursor, statement, args: list = None):
...
#         cursor (pyodbc.connection.cursor) : cursor used to write data
#         statement (str) : statement to execute to write data
#         args (list|None) : arguments to pass to cursor.executemany as placeholder values
...
#     def handle(self, table_name: str, dataframe: pd.DataFrame, error_class: errors, undefined_columns: list):
